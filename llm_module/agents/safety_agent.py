...
def _serialize(result: AnalysisResult) -> dict:
    """AnalysisResult를 pending_actions에 담을 dict로 직렬화."""
    return {
...
async def safety_node(state: FacilityState) -> dict:
    print(f"[AGENT: safety] zone={state['zone_id']}")
    ...
    if result.detected and result.confidence < ESCALATION_THRESHOLD:
        ...
        if frames_b64:
            print(f"[{zone_id}][SAFETY] confidence={result.confidence:.2f} → {ESCALATION_MODEL}로 재판단")
    ...
    if result.severity == Severity.HIGH and result.confidence < 0.80:
        print(f"[{zone_id}][SAFETY] 고위험 불확실({result.confidence:.2f}) → 오케스트레이터 위임")
        return {"conflict_detected": True}

    # 결정만 발행: 알림 + (땀 감지 시) 온도 제어
    actions: list[dict] = [{"kind": "alert", "result": _serialize(result)}]
